...
{
	show_debug_message("game pad connected");
}
else
{
	show_debug_message("game pad not connected");
}
show_debug_message("hascontrol {0}",hascontrol);
if (hascontrol)
{
...
//show_debug_message("*** o_bullet.image_angle= " + string(o_bullet.image_angle));
		...
		ScreenShake(2,10);
		instance_create_layer(x,y,"Bullets",o_muzzle_flash);
		//instance_create_layer(x,y,"Bullets",o_ejected_shell);
...
else
{
	...
	if (hsp == 0) || sliding
	{
		if crouching  
		{
			if sliding
			{
				sprite_index = s_marshal2_slide;
			}
			else
			{
			sprite_index = s_marshal2_crouch;
			mask_index = s_marshal2_crouch;
			}
		}
		else
		{
			sprite_index = s_marshal2_idle;
			mask_index = s_marshal2_idle;
		}
	}
		else
		{
			sprite_index = s_marshal2_run;
					if image_index = 1 {
